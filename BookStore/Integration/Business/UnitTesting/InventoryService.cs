...
public class InventoryService
{
    ...
    public void PrepareDatabase()
    {
       ...
       _productDto = new ProductDto
        ...
        Assert.That(_businessFacade.UsersService.RegisterProvider(_adminUser.Username,_providerUser).IsSuccess, Is.EqualTo(true));
    }
    ...
    {
        Assert.That(_businessFacade.InventoryService.RegisterProduct(_clientUser.Username, _productDto).IsSuccess, Is.EqualTo(false));
    }
    
    [Test, Order(3)]
    public void UnalloyedDeleteProduct()
    {
        Assert.That(_businessFacade.InventoryService.DeleteProduct(_clientUser.Username, _productDto.Name).IsSuccess, Is.EqualTo(false));
    }
    
    [Test, Order(4)]
    public void AllowedToDeleteProduct()
    {
        var delete = _businessFacade.InventoryService.DeleteProduct(_providerUser.Username, _productDto.Name);
